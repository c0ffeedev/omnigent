...
async def _auto_create_codex_terminal(
    ...
    *,
    ...
    if model_credential_env:
        _codex_launch = NativeCodexLaunch(config_overrides=[], model=default_model, profile=None)
    else:
...
async def _auto_create_claude_terminal(
    ...
    *,
    ...
    if server_client is not None and session_external_id is not None:
        ...
        try:
            _resume_kwargs: dict[str, Any] = {}
            if model_credential_env and model_credential_env.get("CLAUDE_CONFIG_DIR"):
                _resume_kwargs["config_dir"] = Path(
                    model_credential_env["CLAUDE_CONFIG_DIR"]
                ).expanduser()
    ...
    env_spec = TerminalEnvSpec(
        ...
        args=launch_args,
        # Tool Search env plus configured-provider or actor-scoped model credentials.
        # Empty provider config still forces ENABLE_TOOL_SEARCH=true so MCP schemas
