...
from omnigent.server.managed_hosts import reserve_managed_host_turn
...
class EventIngestRequest(SessionEventInput):
    """Session event body with optional trusted runner actor context."""
...
def register_events_routes(
    ...
    *,
    ...
    async def post_event(
        ...
        if requested_runner_id is not None:
            if not tunnel_token or token_bound_runner_id(tunnel_token) != requested_runner_id:
        ...
        event_actor_user_id = (
            runner_actor["run_as"] if runner_actor is not None else _attribution_user(user_id)
        )
        # Driver fencing needs the identity recorded by the lease store, not
        # the display-attribution identity above.  The latter intentionally
        # normalizes ``local`` and unauthenticated callers to ``None`` so chat
        # items do not show sentinel actors; leases, however, are acquired as
        # ``local`` or ``__public__`` and require exact principal equality.
        driver_actor_user_id = (
            runner_actor["run_as"] if runner_actor is not None else user_id or RESERVED_USER_PUBLIC
        )
        ...
        _actor = runner_actor or _build_actor(user_id)
        ...
        if body.type == _EXTERNAL_SESSION_STATUS_TYPE:
            ...
            # Preserve the established SessionEventInput wire shape, including
            # nullable model/tool fields.  The runner-only actor extension is
            # request-scoped and must appear only when it was validated above.
            forward_body = body.model_dump(exclude={"actor"})
            if runner_actor is not None:
                forward_body["actor"] = runner_actor
